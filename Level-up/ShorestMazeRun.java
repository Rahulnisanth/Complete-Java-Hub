...
public class ShorestMazeRun {
    ...
    public static void main(String[] args) {
        ...
        int minSteps = Integer.MAX_VALUE;  // Initialize minSteps to maximum value

        // Try all possible starting points (for mazes with multiple solutions)
        for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
        ...
        // Print the solution with minSteps (can be modified to print actual path)
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
    ...
    private static boolean findMaze(int x, int y, int[][] problem, int[][] solution, int steps) {
        ...
        solution[x][y] = steps + 1; // Mark the final step
        return true;
        }
        if (x >= 0 && x < n && y >= 0 && y < n && problem[x][y] == 0 && solution[x][y] == 0) {
        solution[x][y] = steps + 1; // Track steps for this cell
        ...
        // Backtrack (only if no successful path found from this cell)
        solution[x][y] = 0;
        }
        return false;
    }
